...
x_mat <- as.matrix(select(df, d_young, d_mid, d_old, popgrowth))
...
lambda1_samples <- rstan::extract(fit, 'lambda1')$lambda1
lambda2_samples <- rstan::extract(fit, 'lambda2')$lambda2
lambda3_samples <- rstan::extract(fit, 'lambda3')$lambda3
# posterior mean, 95% CI
lambda_est <- data.frame(
  lambda = c('lambda1','lambda2','lambda3'),
  mean   = c(mean(lambda1_samples), mean(lambda2_samples), mean(lambda3_samples)),
  lower  = c(quantile(lambda1_samples,0.025), quantile(lambda2_samples,0.025), quantile(lambda3_samples,0.025)),
  upper  = c(quantile(lambda1_samples,0.975), quantile(lambda2_samples,0.975), quantile(lambda3_samples,0.975))
)
print(lambda_est)
...
effects <- c("young","mid","old","pop_g","immigrant")
...
print(results)
# CSV 저장
write.csv(results, "출력/panel_varx_effects_nooil.csv", row.names = FALSE)
...
tex_file <- "출력/pooled_withoutoil/panel_varx_effects_nooil.tex"
